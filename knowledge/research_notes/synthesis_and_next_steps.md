...
- 下一步如果继续 FSL/Sujianlin 方向，应优先把 event-local decay leftover 做成低维、可解释、endpoint-aware 的 residual add-on，而不是回到 `S1` single-coordinate replacement 或 schedule-only EMA/noise proxy。
...
- 把 `step_plus_event_leftover` 作为下一轮候选：固定保守 step template 后，只加低维 linear-tail/event-local leftover；full/tail 作为主收益，endpoint 作为独立约束或单独校准项。
...
- Sujianlin EMA/history memory、noise-scale proxy、update-RMS proxy 目前是弱正/负结果：它们略优于 momentum baseline，但远弱于 step template，且没有恢复 residual phase。
