...
\begin{frame}{Data Cleaning: Dense Step Grid and Forward Fill}
  \begin{columns}[T,onlytextwidth]
    ...
    \end{column}
  \end{columns}
  \smallnote{Source records: \texttt{loss curves/Readme.txt} and task protocol notes.}
\end{frame}
...
\begin{frame}{Autocorrelation Diagnosis}
  \begin{columns}[T,onlytextwidth]
    ...
    \end{column}
  \end{columns}
  \smallnote{Computed on steps \(t\ge1000\); output: \texttt{loss\_autocorrelation\_metrics.csv}.}
\end{frame}
...
\begin{frame}{Where the Spline Helps: Head and Tail}
  \centering
  \includegraphics[width=0.94\textwidth]{figures/momentum_residual_spline_edge_zoom.png}
  \smallnote{Local zooms follow the multi-panel style of MPL figures: the spline gain is concentrated in the early transient and late WSD decay windows. Shaded bands show the baseline-to-spline gap.}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      \begin{table}
        ...
        \end{tabular}
      \end{table}
      \smallnote{Statistic: \(|e_{\rm momentum}|-|e_{\rm spline}|\). Positive means spline improves.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{table}
        \centering
        ...
        \end{tabular}
      \end{table}
      \smallnote{Placebo templates fail, so the effect is not arbitrary residual magnitude.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Event/Tail Leftover: Incremental Gain}
  \centering
  \includegraphics[width=0.94\textwidth]{figures/step_plus_event_leftover_contrast.png}
  \smallnote{The advanced correction is evaluated relative to the step reference. Positive teal area means the event/tail leftover reduces absolute prediction error beyond the spline-only model.}
\end{frame}

\begin{frame}{Where Event/Tail Helps: Mid and Tail}
  \centering
  \includegraphics[width=0.94\textwidth]{figures/step_plus_event_leftover_edge_zoom.png}
  \smallnote{The gain is localized after the early transient: mid/pre-tail structure improves by 27\%, and WSD decay/tail improves by 13\% over the step reference.}
\end{frame}
...
\begin{frame}{Hypothesis Test II: Does Event Geometry Add Information?}
  ...
  \begin{table}
    ...
    \end{tabular}
  \end{table}
  \smallnote{This narrows the claim: the leftover is useful as a cosine/step-phase tail correction, not as a universal event model from any source schedule.}
\end{frame}
...
\begin{frame}{Event Leftover: Robustness Summary}
  \begin{columns}[T,onlytextwidth]
    ...
    \end{column}
  \end{columns}
  \smallnote{Bootstrap evidence is within-curve support, not schedule-level significance.}
\end{frame}
