...
\newcommand{\MemberA}{占扬 / 2300010819}
\newcommand{\MemberB}{唐锦琪 / 2300010628}
\newcommand{\MemberC}{邓子钧 / 2200015833}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      \begin{table}
        ...
        \end{tabular*}
      \end{table}
      \smallnote[0.92\linewidth]{Source records: \texttt{loss curves/Readme.txt} and task protocol notes.}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      ...
      \begin{table}
        ...
        \end{tabular*}
      \end{table}
      \smallnote[0.86\linewidth]{Computed on steps \(t\ge1000\); output: \texttt{loss\_autocorrelation\_metrics.csv}.}
    \end{column}
    \begin{column}{0.46\textwidth}
...
\begin{frame}{Where the Spline Helps: Head and Tail}
  ...
  \smallnote[0.94\textwidth]{Local zooms follow the multi-panel style of MPL figures: the spline gain is concentrated in the early transient and late WSD decay windows. Shaded bands show the baseline-to-spline gap.}
\end{frame}
...
\begin{frame}{Method II: Event/Tail Leftover}
  ...
  learning-rate schedules \cite{sujianlin2025wdlr}, we model the step
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      ...
      endpoint-loss motivation in \cite{sujianlin2025linear}; we use only LR-derived
...
\begin{frame}{Hypothesis Test II: Does Event Geometry Add Information?}
  ...
  \begin{table}
    \centering
    \scriptsize
    \begin{tabular}{p{0.36\textwidth}p{0.36\textwidth}c}
      \toprule
      test & key numbers & verdict\\
      \midrule
      aligned WSD margin & full \(0.035007\to0.029644\); tail \(0.035694\to0.031012\) & supported\\
      train on 811 only & full \(0.029869\to0.030152\) & not supported\\
      train on cosine+811 & full \(0.031382\to0.029322\) & supported\\
      endpoint guard & endpoint stays \(\approx0.03149\), full \(0.029716\) & mixed\\
      \bottomrule
    \end{tabular}
...
\begin{frame}{Reproducibility}
  ...
  \begin{table}
    ...
    \begin{tabularx}{0.98\textwidth}{l>{\raggedright\arraybackslash}X}
      ...
      \bottomrule
    \end{tabularx}
    \smallnote[0.98\textwidth]{Local environment used during experiments: \texttt{conda activate tdlt}.}
  \end{table}
\end{frame}
...
\begin{frame}{References}
  ...
  \begin{thebibliography}{9}
    ...
    \bibitem{sujianlin2025wdlr}
    苏剑林 (2025).
    \newblock 滑动平均视角下的权重衰减和学习率.
    \newblock 科学空间. \url{https://spaces.ac.cn/archives/11459}.

    \bibitem{sujianlin2025linear}
    苏剑林 (2025).
    \newblock 让炼丹更科学一些（四）：新恒等式，新学习率.
    \newblock 科学空间. \url{https://kexue.fm/archives/11494}.
  \end{thebibliography}
\end{frame}
